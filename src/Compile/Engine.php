...
class Engine extends BuilderHelper
{
    /**
     * @var ?string $query
     */
    ...
    /**
     * Compile ENGINE Blueprint
     * @param ?string $engine Column Name. Example: 'InnoDB'
     * @param string $driver Driver Name. Example: 'mysql'
     * @return string
     */
    public function __construct(?string $engine, string $driver)
    {
        switch ($driver) {
            case 'mysql':
                $engine = !empty($engine) ? $engine : 'InnoDB';
                $this->query = "\nENGINE={$engine}";
                break;
            case 'pgsql':
                $this->query = "";
